...
func (options *GetOptions) Run(f cmdutil.Factory, cmd *cobra.Command, args []string) error {
	...
	var sorter *pi.RuntimeSort
	if len(sorting) > 0 && len(objs) > 1 {
		// TODO: questionable
		if sorter, err = pi.SortObjects(f.Decoder(true), objs, sorting); err != nil {
			return err
		}
	}
	...
	for ix := range objs {
		...
		if resourcePrinter, found := printer.(*printers.HumanReadablePrinter); found {
			...
			if mapping != nil {
				...
				if alias, ok := pi.ResourceShortFormFor(mapping.Resource); ok {
					resourceName = alias
...
func (options *GetOptions) printGeneric(printer printers.ResourcePrinter, r *resource.Result, filterFuncs pi.Filters, filterOpts *printers.PrintOptions) error {
