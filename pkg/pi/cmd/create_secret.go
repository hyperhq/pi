...
var (
	...
	secretExample = templates.Examples(i18n.T(`
	  ...
	  pi create secret generic my-secret --from-file=path/to/bar

	  # Create a new secret named my-secret with specified keys instead of names on disk
	  pi create secret generic my-secret --from-file=ssh-privatekey=~/.ssh/id_rsa --from-file=ssh-publickey=~/.ssh/id_rsa.pub

	  # Create a new secret named my-secret with key1=supersecret and key2=topsecret
	  pi create secret generic my-secret --from-literal=key1=supersecret --from-literal=key2=topsecret

	  # Create a new secret named my-secret using a combination of a file and a literal
	  pi create secret generic my-secret --from-file=ssh-privatekey=~/.ssh/id_rsa --from-literal=passphrase=topsecret

	  # Create a new secret named my-secret from an env file
	  pi create secret generic my-secret --from-env-file=path/to/bar.env`))
)
...
var (
	...
	secretForDockerRegistryExample = templates.Examples(i18n.T(`
		  ...
		  pi create secret docker-registry my-secret --docker-server=DOCKER_REGISTRY_SERVER --docker-username=DOCKER_USER --docker-password=DOCKER_PASSWORD --docker-email=DOCKER_EMAIL`))
)

// NewCmdCreateSecretDockerRegistry is a macro command for creating secrets to work with Docker registries
func NewCmdCreateSecretDockerRegistry(f cmdutil.Factory, cmdOut io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "docker-registry NAME --docker-username=user --docker-password=password --docker-email=email [--docker-server=string]",
...
func CreateSecretDockerRegistry(f cmdutil.Factory, cmdOut io.Writer, cmd *cobra.Command, args []string) error {
	...
	var generator pi.StructuredGenerator
	switch generatorName := cmdutil.SecretForDockerRegistryV1GeneratorName; generatorName {
	case cmdutil.SecretForDockerRegistryV1GeneratorName:
		generator = &pi.SecretForDockerRegistryGeneratorV1{
			Name:       name,
...
var (
	...
	secretForTLSExample = templates.Examples(i18n.T(`
	  ...
	  pi create secret tls tls-secret --cert=path/to/tls.cert --key=path/to/tls.key`))
)
...
func CreateSecretTLS(f cmdutil.Factory, cmdOut io.Writer, cmd *cobra.Command, args []string) error {
	...
	case cmdutil.SecretForTLSV1GeneratorName:
		generator = &pi.SecretForTLSGeneratorV1{
			Name:       name,
