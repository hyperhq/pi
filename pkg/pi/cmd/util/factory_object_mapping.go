...
import (
	...
	openapivalidation "github.com/hyperhq/pi/pkg/pi/cmd/util/openapi/validation"
...
func (f *ring1Factory) Reaper(mapping *meta.RESTMapping) (pi.Reaper, error) {
	mappingVersion := mapping.GroupVersionKind.GroupVersion()
	clientset, clientsetErr := f.clientAccessFactory.ClientSetForVersion(&mappingVersion)
	reaper, reaperErr := pi.ReaperFor(mapping.GroupVersionKind.GroupKind(), clientset)

	if pi.IsNoSuchReaperError(reaperErr) {
...
func (f *ring1Factory) HistoryViewer(mapping *meta.RESTMapping) (pi.HistoryViewer, error) {
	external, err := f.clientAccessFactory.KubernetesClientSet()
	if err != nil {
		return nil, err
	}
	return pi.HistoryViewerFor(mapping.GroupVersionKind.GroupKind(), external)
}
