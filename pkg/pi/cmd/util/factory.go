...
// Factory provides abstractions that allow the pi command to be extended across multiple types
...
type DiscoveryClientFactory interface {
	...
	// BindFlags adds any discovery flags that are common to all pi sub commands.
...
type ClientAccessFactory interface {
	...
	// BindFlags adds any flags that are common to all pi sub commands.
	BindFlags(flags *pflag.FlagSet)
	...
	DefaultResourceFilterFunc() pi.Filters
	...
	Generators(cmdName string) map[string]pi.Generator
...
type ObjectMappingFactory interface {
	...
	Reaper(mapping *meta.RESTMapping) (pi.Reaper, error)
	// Returns a HistoryViewer for viewing change history
	HistoryViewer(mapping *meta.RESTMapping) (pi.HistoryViewer, error)
