...
func (f *discoveryFactory) DiscoveryClient() (discovery.CachedDiscoveryInterface, error) {
	...
	cacheDir := computeDiscoverCacheDir(filepath.Join(homedir.HomeDir(), ".pi", "cache", "discovery"), cfg.Host)
...
func (f *ring0Factory) MapBasedSelectorForObject(object runtime.Object) (string, error) {
	...
	case *api.ReplicationController:
		return pi.MakeLabels(t.Spec.Selector), nil
	case *api.Pod:
		if len(t.Labels) == 0 {
			return "", fmt.Errorf("the pod has no labels and cannot be exposed")
		}
		return pi.MakeLabels(t.Labels), nil
	case *api.Service:
	...
	case *extensions.Deployment:
		...
		return pi.MakeLabels(t.Spec.Selector.MatchLabels), nil
...
func (f *ring0Factory) DefaultResourceFilterFunc() pi.Filters {
	return pi.NewResourceFilter()
}
...
func DefaultGenerators(cmdName string) map[string]pi.Generator {
	...
	case "secret":
		generator = map[string]pi.Generator{
			SecretV1GeneratorName: pi.SecretGeneratorV1{},
		}
...
func (f *ring0Factory) Generators(cmdName string) map[string]pi.Generator {
