...
//
// This method is generic to the command in use and may be used by non-pi
// commands.
...
// func with that string and an pi exit code.
...
func AddIncludeUninitializedFlag(cmd *cobra.Command) {
	cmd.Flags().Bool(IncludeUninitializedFlag, false, `If true, the pi command applies to uninitialized objects. If explicitly set to false, this flag overrides other flags that make the pi commands apply to uninitialized objects, e.g., "--all". Objects with empty metadata.initializers are regarded as initialized.`)
}
...
func AddApplyAnnotationFlags(cmd *cobra.Command) {
	cmd.Flags().Bool(ApplyAnnotationsFlag, false, "If true, the configuration of current object will be saved in its annotation. Otherwise, the annotation will be unchanged. This flag is useful when you want to perform pi apply on this object in the future.")
}

func AddApplyAnnotationVarFlags(cmd *cobra.Command, applyAnnotation *bool) {
	cmd.Flags().BoolVar(applyAnnotation, ApplyAnnotationsFlag, false, "If true, the configuration of current object will be saved in its annotation. Otherwise, the annotation will be unchanged. This flag is useful when you want to perform pi apply on this object in the future.")
}
...
func AddRecordFlag(cmd *cobra.Command) {
	cmd.Flags().Bool("record", false, "Record current pi command in the resource annotation. If set to false, do not record the command. If set to true, record the command. If not set, default to updating the existing annotation value only if one already exists.")
}

func AddRecordVarFlag(cmd *cobra.Command, record *bool) {
	cmd.Flags().BoolVar(record, "record", false, "Record current pi command in the resource annotation. If set to false, do not record the command. If set to true, record the command. If not set, default to updating the existing annotation value only if one already exists.")
}
...
func RecordChangeCause(obj runtime.Object, changeCause string) error {
	...
	annotations[pi.ChangeCauseAnnotation] = changeCause
...
func ContainsChangeCause(info *resource.Info) bool {
	...
	return len(annotations[pi.ChangeCauseAnnotation]) > 0
}
...
func MustPrintWithKinds(objs []runtime.Object, infos []*resource.Info, sorter *pi.RuntimeSort) bool {
...
func FilterResourceList(obj runtime.Object, filterFuncs pi.Filters, filterOpts *printers.PrintOptions) (int, []runtime.Object, error) {
