...
func AddHandlers(h printers.PrintHandler) {
	...
	serviceColumnDefinitions := []metav1alpha1.TableColumnDefinition{
		...
		{Name: "LoadBalancer-IP", Type: "string", Description: apiv1.ServiceSpec{}.SwaggerDoc()["loadBalancerIP"]},
...
func getServiceLoadBalancerIP(svc *api.Service, wide bool) string {
	switch svc.Spec.Type {
	case api.ServiceTypeClusterIP:
		return "<none>"
	case api.ServiceTypeNodePort:
...
func printService(obj *api.Service, options printers.PrintOptions) ([]metav1alpha1.TableRow, error) {
	...
	externalIP := getServiceLoadBalancerIP(obj, options.Wide)
